...
class EnhancedDataLoader:
    ...
    def _format_datetime_for_neo4j(self, dt):
        """Convert datetime to Neo4j compatible format"""
        if pd.isna(dt):
            return None
        ...
        # Format for Neo4j: YYYY-MM-DDTHH:MM:SS
    ...
    def load_uci_retail_fixed(self):
        ...
        try:
            ...
            purchases['InvoiceDate'] = purchases['InvoiceDate'].apply(self._format_datetime_for_neo4j)
            purchases = purchases.dropna(subset=['InvoiceDate'])

            # Load purchases in batches
            with self.driver.session() as session:
                for i in tqdm(range(0, min(len(purchases), 50000), self.batch_size)):
                    batch = purchases.iloc[i:i + self.batch_size]
                    batch_dict = batch.to_dict('records')
    ...
    def load_sessions_and_clickstream(self):
        ...
        try:
            ...
            df['event_time_str'] = df['event_time'].apply(self._format_datetime_for_neo4j)

            # Create sessions
            sessions = df.groupby('user_session').agg({
                'user_id': 'first',
                'event_time_str': ['min', 'max'],
                'event_type': 'count'
            }).reset_index()
